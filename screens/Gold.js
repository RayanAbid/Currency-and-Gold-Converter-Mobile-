...
export default function App({ navigation, route }) {
  ...
  const GetRate = async () => {
    await fetch("http://goldprices.herokuapp.com/goldprice", {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
      },
      body: { country: "Afghanistan", scale: "tola" },
    })
      .then((res) => res.json())
      .then((resjson) => {
        setConvertedVal(resjson);
        setVisible(false);
        console.log("currency", resjson);
      })
      .catch(function (error) {
        setVisible(false);
        alert("Sorry !");
        console.error("bccc", error);
      });
  };
  ...
  //   const options = {
  //     method: "POST",
  //     url: "http://goldprices.herokuapp.com/goldprice/",
  //     params: { country: "Afghanistan", scale: "tola" },
    ...
    <>
      <Block flex center style={styles.home}>
        ...
        <Block row style={{ width: "100%" }}>
          <Button
            onPress={() =>
              navigation.navigate("SelectCurrency", { dataFor: "convertFrom", pageFrom: "Gold" })
            }>
            Convert From
          </Button>
